...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14932/badge)](https://www.bestpractices.dev/projects/14932)
<!-- badges: end -->
...
Because some members are not yet on CRAN, `seor` is installed from GitLab for
...
The `env =` argument disables only the CRAN incoming-feasibility step. seor is
a metapackage with members that are not yet on CRAN, so that step always
reports an ERROR (non-mainstream dependencies, the `Remotes:` field, the
archived `seoR` name clash) — a permanent, known state rather than a
regression. Every other `--as-cran` check still runs. Re-enable it for the
